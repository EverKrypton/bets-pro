      ...
      {isOpen && (
        <>
          ...
          >
            <button
              onClick={onClose}
              className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-colors z-10"
            >
              <X size={16}/>
            </button>

            <div className="relative">
              <div className={`h-32 bg-gradient-to-br ${steps[step].gradient} relative overflow-hidden`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  >
                    {(() => {
                      ...
                      return <IconComponent size={48} className="text-white/90" />;
                    })()}
                  </motion.div>
                </div>
              </div>
              ...
              <div className="p-6">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <h2 className="text-xl font-black text-white mb-1">{steps[step].title}</h2>
                    <p className="text-sm text-gray-400 mb-3">{steps[step].subtitle}</p>
                    {steps[step].content && (
                      <p className="text-gray-300 text-sm leading-relaxed mb-4">{steps[step].content}</p>
                    )}
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-white/5">
                              <th className="py-2 px-3 text-left text-gray-400 font-bold">Deposit</th>
                              <th className="py-2 px-3 text-right text-gray-400 font-bold">Bonus</th>
                            </tr>
                          </thead>
                          <tbody>
                            {steps[step].table.map((row, i) => (
                              <tr key={i} className="border-t border-white/5">
                                <td className="py-2 px-3 text-white font-bold">{row.deposit}</td>
                                <td className="py-2 px-3 text-right text-green-400 font-black">{row.bonus}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        ...
                        {steps[step].requirements.map((req, i) => (
                          ...
                          >
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center">
                              <req.icon size={18} className="text-white/70"/>
                            </div>
                            <span className="text-sm text-white font-medium">{req.text}</span>
                          </motion.div>
                        ))}
                      </div>
                    )}
