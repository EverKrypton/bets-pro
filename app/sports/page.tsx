...
import { Clock, X, ChevronDown, ChevronUp, Info } from 'lucide-react';
...
function OddBtn({
  label, odd, active, onClick, disabled = false,
}: {
  label: string; odd: number; active: boolean;
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        flex-1 flex flex-col items-center justify-center py-2.5 rounded-lg border
        transition-all active:scale-95 select-none
        ${active
          ? 'bg-accent border-accent text-white'
          : 'bg-background border-white/8 hover:border-accent/50 hover:bg-accent/5'
        }
        ${disabled ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}
      `}
    >
      <span className={`text-[10px] font-bold uppercase leading-none mb-0.5 ${active ? 'text-white/60' : 'text-gray-500'}`}>
        {label}
      </span>
...
export default function SportsPage() {
  ...
  // Bet slip — single bet only (clean like 1win mobile)
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setMatches(list);
      const uniq = Array.from(new Set(list.map(m => m.league).filter(Boolean)));
      setLeagues(uniq);
  ...
  const pickOdd = (match: Match, selection: string, odd: number) => {
    ...
    setSlip({
      ...
      league:    match.league,
      selection,
      label:     MARKET_FULL[selection] ?? selection,
      odd,
    });
    setAmount('');
    setFeedback(null);
  ...
  const placeBet = async () => {
    ...
    if (!amt || amt < minBet) {
      setFeedback({ text: `Min bet: ${minBet} USDT`, ok: false });
      return;
    }
    if (amt > maxBet) {
      setFeedback({ text: `Max bet: ${maxBet} USDT`, ok: false });
      return;
    }
  ...
  const potWin = slip && Number(amount) > 0
  ...
  const STATUS_BG: Record<string, string> = {
    ...
    refunded: 'text-gray-400 bg-white/5',
  };
    ...
    <Layout>
                  ...
                  {myBets.map(b => (
                    <div key={b._id} className="px-4 py-3 flex justify-between items-center gap-3">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xs font-black">{Number(b.amount).toFixed(2)} USDT</p>
                        <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded-full ${STATUS_BG[b.status] ?? ''}`}>
                          {b.status}
                        </span>
                      </div>
                    </div>
      ...
      <AnimatePresence>
        {slip && (
          ...
          >
            <div className="bg-[#1a1f2e] border border-accent/40 rounded-2xl shadow-2xl overflow-hidden">
              {/* Slip header */}
              <div className="flex items-center justify-between px-4 py-3 bg-accent/10 border-b border-accent/20">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {slip.label}
                    <span className="ml-2 text-white font-black">{slip.odd.toFixed(2)}</span>
                  </p>
                </div>
                <button
                  onClick={() => setSlip(null)}
                  className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-gray-400 hover:text-white shrink-0 ml-2"
                >
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="p-4 space-y-3">
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg border text-xs font-black transition-all ${
                        Number(amount) === v
                          ...
                          : 'bg-surface border-white/8 text-gray-500 hover:text-white'
                      }`}
                    >
                      {v}
                    </button>
                ...
                <div className="flex justify-between items-center px-1">
                  ...
                  <span className={`font-black text-base ${potWin ? 'text-green-400' : 'text-gray-600'}`}>
                    {potWin ? `${potWin} USDT` : '—'}
                  </span>
                </div>
                ...
                <button
                  onClick={placeBet}
                  disabled={placing || !amount || Number(amount) < minBet}
                      ...
                      {valid ? (
                        <div className="px-3 pb-3 space-y-2">
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1 py-1.5 text-[10px] font-bold text-gray-600 hover:text-gray-400 transition-colors"
                          >
                            Double Chance {dcOpen ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
                          </button>
