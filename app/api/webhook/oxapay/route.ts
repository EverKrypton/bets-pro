/**
 * OxaPay Webhook - Improved Error Handling
 */
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    console.log('[OxaPay] Headers:', Object.fromEntries(req.headers.entries()));
    ...
    console.log('[OxaPay] Parsed:', { type, status, track_id: body.track_id, order_id: body.order_id, amount: body.amount });

    const secret = type && PAYMENT_TYPES.has(type) ? MERCHANT_KEY: type === 'payout'               ? PAYOUT_KEY: undefined;

    if (!secret) {
      console.error('[OxaPay] Unknown type:', type);
      return bad();
    }
    ...
    console.log('[OxaPay] Processing:', { type, status, track_id, order_id, address, amount });
    ...
    if (type && PAYMENT_TYPES.has(type) && status === 'Paid') {
      ...
      await user.save();
      console.log('[OxaPay] Updated balance:', user.balance);
    ...
    console.log('[OxaPay] Webhook processed successfully');
    return ok();
